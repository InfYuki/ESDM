...
class TransformerEncoderLayer(Module):
    ...
    def __init__(
        ...
        if use_diff_attn:
            print("Using Differentiable Attention")
            self.self_attn = Diff_MultiheadAttention(
                d_model,
                nhead,
                dropout=dropout,
...
''''''
class Diff_MultiheadAttention(Module):
    r"""Multihead attention (Diff + low-memory KV) with pairwise_features support."""
    def __init__(
        self,
        embed_dim,
        num_heads,
        ...
        self.additive_attn = additive_attn  # 保留参数但不再使用 additive
        self.pairwise_featurization = pairwise_featurization
        ...
        # 差分注意力的两路 Q/K + 一路 V
        self.linear_q1 = Linear(embed_dim, embed_dim, **factory_kwargs)
        self.linear_k1 = Linear(embed_dim, embed_dim, **factory_kwargs)
        self.linear_q2 = Linear(embed_dim, embed_dim, **factory_kwargs)
        self.linear_k2 = Linear(embed_dim, embed_dim, **factory_kwargs)
        self.linear_v = Linear(embed_dim, embed_dim, **factory_kwargs)

        self.att_dropout = Dropout(dropout)
        self.layer_norm = LayerNorm(embed_dim, **factory_kwargs)
        self.out_proj = NonDynamicallyQuantizableLinear(embed_dim, embed_dim, bias=True, **factory_kwargs)

        # 差分注意力门控参数
        self.lambda_init = 0.8  # 对应 lambda_init_fn(0)
        self.lambda_q1 = Parameter(torch.zeros(self.embed_dim).normal_(mean=0.0, std=0.1))
        self.lambda_k1 = Parameter(torch.zeros(self.embed_dim).normal_(mean=0.0, std=0.1))
        self.lambda_q2 = Parameter(torch.zeros(self.embed_dim).normal_(mean=0.0, std=0.1))
        self.lambda_k2 = Parameter(torch.zeros(self.embed_dim).normal_(mean=0.0, std=0.1))
    ...
    def multi_head_attention_forward(
        ...
        #print(">>> DIFF ATTENTION ACTIVE", query.shape)
        ...
        assert head_dim * num_heads == embed_dim, "embed_dim not divisible by num_heads"

        # 差分注意力的 Q/K/V
        q1 = self.linear_q1(query)
        k1 = self.linear_k1(key)
        q2 = self.linear_q2(query)
        k2 = self.linear_k2(key)
        v = self.linear_v(value)

        # reshape to (B*H, L, D)
        q1 = q1.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
        k1 = k1.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
        q2 = q2.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
        k2 = k2.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
        v = v.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)

        # pairwise_features: (B, L, L, H) -> (B*H, L, L, D) 对齐当前实现
        if pairwise_features is not None:
            pairwise_features = pairwise_features.permute(1, 2, 0, 3).contiguous()
            pairwise_features = pairwise_features.view(tgt_len, tgt_len, bsz * num_heads, head_dim)
            pairwise_features = pairwise_features.permute(2, 0, 1, 3)
            # 低内存处理：用 pairwise 的平均作为 K 的修正项
            pairwise_delta = pairwise_features.mean(dim=2)  # (B*H, L, D)
            k1 = k1 + pairwise_delta
            k2 = k2 + pairwise_delta

        # key_padding_mask: True 为 padding，mask 掉 k/v
        if key_padding_mask is not None:
            assert key_padding_mask.shape == (bsz, tgt_len)
            mask = key_padding_mask[:, None, :].expand(bsz, num_heads, tgt_len).reshape(bsz * num_heads, tgt_len)
            k1 = k1.masked_fill(mask[:, :, None], 0.0)
            k2 = k2.masked_fill(mask[:, :, None], 0.0)
            v = v.masked_fill(mask[:, :, None], 0.0)

        # dropout 仅在训练时
        if not training:
            dropout_p = 0.0

        # 低内存：先算 K^T V，再算 Q·(K^T V)
        k1v = k1.transpose(1, 2).matmul(v)  # (B*H, D, D)
        k2v = k2.transpose(1, 2).matmul(v)

        if dropout_p > 0.0:
            k1v = F.dropout(k1v, p=dropout_p)
            k2v = F.dropout(k2v, p=dropout_p)
        ...
        x1 = q1.matmul(k1v)  # (B*H, L, D)
        x2 = q2.matmul(k2v) * lambda_full
        x = x1 - x2
