...
def get_spec_featurizer(spec_features, **kwargs):
    return {
        ...
        "dreams": DreaMSPeakFeaturizer,  # 新增
        "dreams_cache": DreaMSCacheFeaturizer,
...
class DreaMSPeakFeaturizer(SpecFeaturizer):
    """
    DreaMS-style raw mz + intensity featurizer.
    Produces:
      - mz: [N]
      - intens: [N] normalized
      - precursor_mz: float
    """
    def __init__(self, max_peaks=100, **kwargs):
        super().__init__(**kwargs)
        self.max_peaks = max_peaks

    def _featurize(self, spec: data.Spectra, **kwargs) -> Dict:
        # raw spectra (mz, intensity)
        spectra_ar = spec.get_spec()
        merged = utils.merge_norm_spectra(spectra_ar)

        if merged.shape[0] == 0:
            mz = np.zeros((0,), dtype=np.float32)
            intens = np.zeros((0,), dtype=np.float32)
        else:
            # sort by intensity desc, keep top-k
            merged = merged[merged[:, 1].argsort()[::-1]]
            if self.max_peaks is not None:
                merged = merged[: self.max_peaks]
            mz = merged[:, 0].astype(np.float32)
            intens = merged[:, 1].astype(np.float32)

        precursor_mz = getattr(spec, "parentmass", 0.0)
    ...
    def collate_fn(input_list: List[dict]) -> Dict:
        names = [j["name"] for j in input_list]
        mz_list = [torch.from_numpy(j["mz"]) for j in input_list]
        intens_list = [torch.from_numpy(j["intens"]) for j in input_list]
        precursor_mz = torch.tensor([j["precursor_mz"] for j in input_list], dtype=torch.float32)

        lengths = torch.tensor([len(x) for x in mz_list], dtype=torch.long)
        ...
        mz_pad = torch.stack([pad_1d(x.float(), max_len) for x in mz_list], dim=0)
        intens_pad = torch.stack([pad_1d(x.float(), max_len) for x in intens_list], dim=0)
...
class DreaMSCacheFeaturizer(SpecFeaturizer):
    ...
    def _featurize(self, spec: data.Spectra, **kwargs) -> Dict:
        spec_name = spec.get_spec_name()
        path = Path(self.cache_dir) / f"{spec_name}.pt"
        if not path.exists():
            raise FileNotFoundError(f"Missing cached feature: {path}")
        d = torch.load(path, map_location="cpu")
        return {
            "mz": d["mz"].numpy(),
            "intens": d["intens"].numpy(),
            "precursor_mz": float(d["precursor_mz"].item()),
            "name": spec_name,
        }

    @staticmethod
    def collate_fn(input_list: List[dict]) -> Dict:
        # 与 DreaMSPeakFeaturizer 的 collate 一致
        names = [j["name"] for j in input_list]
        mz_list = [torch.from_numpy(j["mz"]) for j in input_list]
        intens_list = [torch.from_numpy(j["intens"]) for j in input_list]
        precursor_mz = torch.tensor([j["precursor_mz"] for j in input_list], dtype=torch.float32)
